...
We generate several candidate LCA models using the randomLCA package (Beath KJ. randomLCA: An R Package for Latent Class with Random Effects Analysis. Journal of Statistical Software, 81(13):1-25)
```{r,include=FALSE}
# Load the corneal ulcer dataset
ulcer_filepath <- '/Users/redd/Library/Mobile Documents/com~apple~CloudDocs/Desktop/Latent Supervision Trachoma/Latent supervision code repo/corneal ulcer dataset.csv'
...
```
```{r}
two_class_random_LCA
```
```{r}
two_class_random_const_LCA
```
...
In this example we will use Keras to train a simple neural network in R, but any classifier can be trained using the probabilistic labels obtained in Step 2 and the modified MSE loss function defined below
```{r}
# library(keras)
# 
# rescaled_MSE <- function(y_true, y_pred) {
#   mse <- k_square(y_true - y_pred)
#   rescaled_mse <- mse * 10
#   loss <- k_mean(rescaled_mse, axis = -1)
#   return(loss)
# }
# 
...
# 
# # Compile the model using the rescaled MSE loss function
# model %>% compile(
#   loss = rescaled_MSE,
...
trachoma_filepath <- '/Users/redd/Library/Mobile Documents/com~apple~CloudDocs/Desktop/Latent Supervision Trachoma/Latent supervision code repo/trachoma dataset.csv'
...
```
### Step 3
The same principles apply as in the infectious keratitis example described above
